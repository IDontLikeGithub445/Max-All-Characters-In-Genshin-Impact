...
CharactersList = {
    ...
    "Rosaria": ("Cryo", "Shadow", "Ballad", "Primordial Jade Winged-Spear", "Guyun"), #I just put her in the Minorities, so Shadow had the least people, Ballad has the least people, and using Primo Geovishap things had the least people.
...
for x in range(len(CharactersList)):#Do for every character in the Dictionary
    ...
    while not Boss_Check(WiiSports): #Check if we have enough weekly boss materials
        ...
        #if Resin_Count >= 60:#Murder one boss per week, instead of all 3 a week. Saves 120 resin worth of time, if you're only trying to max one character
        #    Boss_Loot(WiiSports)
        #    Weeks_Waited += 1
        #    Resin_Count -= 60
        ...
        if Resin_Count >= 180:#Murder all three bosses per week, instead of 1. Yes I know the max resin is 160, but the time waited doesnt differ.
            Boss_Loot("ALL")
            Weeks_Waited += 1
            Resin_Count -= 180
            Mora_Cost -= 24000
