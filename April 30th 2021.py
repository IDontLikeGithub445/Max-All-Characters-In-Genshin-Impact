...
#Options are 1 and 4 respectfully
Boss_Method = 4
...
#Include Mora from Daily Coms
#Options are True or False
Include_DailyMora = False
...
CharactersList = {
    ...
    "Eula":("Cryo", "Crown", "Dilligence", "Wolf's Gravestone", "Dandelion"), #Taking guesses again
...
print("     Include Mora from Daily Commisions: " + str(Include_DailyMora))
print("")
print("")
...
for _ in range(len(CharactersList)):#Do for every character in the Dictionary
    Character_EXP = 0
    Name = random.choice(list(CharactersList.keys()))#Do in a random order just for entertainment purposes
...
#Pre-Mora Blossom Daily Com Check
Mora_Cost2 = Mora_Cost
Days_Spent_Resin = round(Minutes_Spent_Resin/1440, 2)
if Include_DailyMora:
    Mora_Cost2 -= Days_Spent_Resin * 22550# I explain this later, but I just added both the 4 dailys, plus the extra reward together
...
#Daily Commisions
Adventure_EXP += Days_Spent_Resin * 1000
Companionship_EXP += Days_Spent_Resin * 180 #2 NPC comms, worth 35, and 2 generic quests, "Kill Quests" as I'll call them, worth 55
if Include_DailyMora:
    Mora_Cost -= Days_Spent_Resin * 10225 #^
Primogems += Days_Spent_Resin * 40

#I could do it all at once, but Im going to split it so its easier to recognize visually. This is the Daily Commisions Reward
Adventure_EXP += Days_Spent_Resin * 500
Companionship_EXP += Days_Spent_Resin * 100 #2 NPC comms, worth 35, and 2 generic quests, "Kill Quests" as I'll call them, worth 55
if Include_DailyMora:
    Mora_Cost -= Days_Spent_Resin * 12325#^
Primogems += Days_Spent_Resin * 20

while Mora_Cost < -60000:#Ok fine i'll slightly optimize this bit
    Minutes_Spent_Resin -= 8 * 20
    Total_Resin -= 20
    Mora_Blossom_Count -= 1
    Mora_Cost += 60000
    Adventure_EXP -= 100
